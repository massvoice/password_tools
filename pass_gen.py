...
extra_bit = []
temp = []
simple_pass_generated = ""
complex_pass_generated = ""
...
# Fuction to generate custom pasword based upon the user's requirement
	...
	def custom_password_generation(self):
		pass_type = int(raw_input("Which type of password you wish to have? \nPress 1 for Simple \nPress 2 for Complex\n> "))
		char_choice = int(raw_input("\nPress 1 If you wish to specify the type and number of characters you want \nPress 2 If you wish the computer to do it for you\n> "))

		if char_choice == 2:
			pass_length = int(raw_input("\nPlease specify the password length ( between 8-20)\n> "))
			if pass_type == 1 and pass_length in range (8,20):
				simple_pass_generated =  self.simple_password(int(pass_length))
				print (simple_pass_generated)
				
			elif pass_type == 2 and pass_length in range (8,20):
				print (self.complex_password(int(pass_length)))
			else:
				print ("Wrong Input!!")

		elif char_choice == 1 and pass_type == 1:
			user_lower = int(raw_input("Please enter the number of lower case characters\n> "))
			user_upper = int(raw_input("Please enter the number of upper case characters\n> "))
			...
			user_lower = int(raw_input("Please enter the number of lower case characters\n> "))
			user_upper = int(raw_input("Please enter the number of upper case characters\n> "))
			user_numeric = int(raw_input("Please enter the number of numeric characters\n> "))
			user_special_char = int(raw_input("Please enter the number of special characters\n> "))
			for i in range (0, user_lower):
				lower_case.append(random.choice(lower_case_list))
			for i in range (0, user_upper):
				upper_case.append(random.choice(upper_case_list))
...
# Automatically generate password for the user, takes two parameters: pass_type(can be either simple or complex) and length
	def automated_pass_generation(self,pass_type,length=8):
		
		if length < 8:
			length = 8
			print ("Minimum Length required is 8")
		if pass_type == "simple":
			return self.simple_password(length)
		elif pass_type == "complex":
			return self.complex_password(length)
		else:
			return "Wrong Input"
